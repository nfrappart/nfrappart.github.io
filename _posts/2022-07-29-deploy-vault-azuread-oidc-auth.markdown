...
Since I pass most of my days on azure the obvious IDP is Azure AD. To connect with external IDP, vault theses `auth methods` and there are [quite a few available](https://www.vaultproject.io/docs/auth). To use Azure AD, we have to rely, not on ldap or kerberos, but on Open ID Connect (OIDC). I know, I know, naming their IDP Azure *Active directory* was such a bad move, because it's so much more than a AD. :p
...
We have to create an Azure AD application, with proper settings for OIDC, and adequat microsoft graph api permissions. This can be easily done via Azure portal or azCLI `az ad` commands, but I'm stubborn and will go the `Terraform` road. ^_^
...
You will also need and existing Azure Keyvault.
...
I strongly suggest you pin the provider version to avoid issues later on with new version breaking changes.
...
Vault needs you to provide valid token and an api address. To do  have to setup `VAULT_ADDR` environment variable as `https://myvault.domain.com:port`. As for the token, if you've just bootstrap your cluster, a simple `vault login` then providing the `root token` to the prompt will do the trick.
...
However the `appRoles` needs more explanation. As you can see in the default value it's a nested map. The idea is to have your app roles names as key, and a description as well as a list of (vault) policies as key/value pairs. This variable is used to create both app roles in azure AD application, and external groups identity in vault.
...
### *Data Sources*
We will need a few data sources. The `azurerm_client_config` is in almost all my TF projects as it allows me to easily retrieve subscription and tenant information.
...
Now we're getting serious, and work on the actual stuff: the Azure AD App Registration.
...
This is the application itself. You can see that we limit the sigin to user from our own tenant only with `sign_in_audience = "AzureADMyOrg"`. Also the `app_role` block is dynamic and loops over the `appRoles` input variable seen above. This does 2 things for us: keep the code DRY and allow easier and more flexible updates to your role management when you decide to change or add app roles. <br>
Very important point is the oidc claim `group_membership_claims = ["ApplicationGroup"]`. Instead of claiming groups or security groups (I don't remember which one, to be honest), we ask for the `ApplicationGroup` which represents the azure ad application app roles.<br>
The `required_resource_access` block is where you can set either delegated or application permissions. For this specific usage, I find the delegated permission with admin consent is enough, as there shouldn't be any request from vault using this principal, without a auth request from a user.
...
So, we have an azure AD application. great!
Now what?
...
To make this possible Vault needs a principal and secret to authenticate as the application we just created. Additionnaly we need to grant this principal some permissions to go through with a user login process. <br>
So far, with the delegated approach (see Azure documentation for details between application and delegated permissions) a simple `User.Read` claim proved sufficient. Since a reader can read its own groups and roles, Vault doesn't need more. Of course if we wanted vault to manage Azure AD ressources as a backend service, without a user logged in, that would be a different story.
To keep track of the application secret, we create an azure keyvault secret. We could also store it in a Vault kv secret engine, but I assume if you are setting up your auth methods, you don't have secret engines yet.
...
We finally get into Vault configuration itself. Regarding the auth method ther isn't that much to do actually. Just 2 things:
- create backend
...
The path is where the auth method mount will be set in Vault API. `oidc` is the default, so I kept it the same. We could go a step more and use an input variable to be able to easily customize it, but we'd need to substitute it in the app registration redirect URIs accordingly. Same goes for the URIs set in the backend role below.

As you can see, we retrieve the secret from the keyvault resource (from the state). 

Regarding the `vault_jwt_auth_backend_role` there are a few key points.<br>
First, `token_policies` provides a list of the default policies anyone successfully logging will be assigned. The default policy is not a bad place to start so you can login and see minor things. A better way would be to create a minimalistic policy and use it as default for this role.<br>
...
External groups are provisioned from the appRoles variable. If you remember what we said earlier, this variable gives the name of the role and assign a list of policies for said role.
